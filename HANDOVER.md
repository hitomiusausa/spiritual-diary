...
1. 規約・プライバシー・サポートの運営者名と問い合わせ窓口の記入（ユーザー判断待ち）
2. Webのデプロイ先とiOSの器の方式決定（ユーザー判断待ち。/api があるためNodeホスティングが前提）
3. 四柱推命の専門家・外部命式表との人による照合(独立演算照合はD-10で済。公開前推奨）
4. `/api/analyze` と `/api/chat` の認証（レート制限・日次上限はD-08で第一段階済み。永続的な利用量監視は未実装）
5. チャットの購読状態チェックとStoreKit/RevenueCat連携（`checkChatEntitlement()`内に実装する）
6. iOSの器、App Store素材、実機iPhone QA
7. 課金導入時: 会話ログ閲覧の有料ゲート（D-11）とStoreKit/RevenueCatのentitlement接続

## 次に進める順番

1. 運営者名・問い合わせ窓口を確定して法務3ページに記入する
2. Webのデプロイ先を決めて公開する（iOSの器はAPIサーバーの公開が前提）
3. iOS化と課金を実装する（entitlementはD-08のスタブに接続、閲覧境界はD-11、バックアップはD-09第二段階へ）
