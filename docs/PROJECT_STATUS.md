...
- 入力値の形式・範囲・本文長をサーバー側でも検証する（分析APIは実装済み。プレースホルダーAPIは共通化予定）
...
- プレースホルダーAPIも`src/lib/saju.js`を使う計算へ統一し、旧来の簡易大運計算と文字列正規表現抽出を廃止
- 検証: `npm test` 12件成功、`npm run build`成功、`npm run lint`エラーなし、`npm audit --omit=dev`脆弱性0件
