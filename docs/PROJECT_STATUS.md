# Kiri / Mind & Energy Note — 現状と次の作業

更新日: 2026-07-28

## 1. このリポジトリの位置づけ

`spiritual-diary`がKiriサービスの正規本体です。旧`kiri-chat`（`Documents/Playground/kiri-chat`）は、Kiriとの自由チャット人格・安全性・チャットUIを検証した別プロトタイプです。両方を無計画に混ぜず、占い結果を中心とする本体へ、必要なチャット機能だけを後から統合します。

## 2. 現在の構成

```text
src/app/page.js
  └─ SpiritualDiary
       ├─ start: ニックネーム・出生情報
       ├─ input: 気分・出来事・直感
       ├─ loading: Kiriの分析演出
       └─ result: 命式・運勢・ヒント・Kiriメッセージ・端末内履歴

src/app/api/analyze/route.js
  ├─ lunar-javascriptで出生時と現在の四柱を算出
  ├─ 五行相性、テーマ別スコア、今日のヒント、大運を算出
  └─ Anthropic APIでdeepMessage / innerMessage / actionAdviceを生成

src/app/api/chat/route.js
  └─ 占い結果を文脈に渡すKiriチャット（開発プレビュー）

src/lib/kiriPersonality.js
  └─ チャット版と占い版で共有するKiriの人格定義
src/lib/saju.js
  └─ 入力検証、八字、現在の四柱、大運の純粋計算

src/components/SpiritualDiary.jsx
  └─ UI、入力状態、端末内履歴を管理

src/lib/history.js
  └─ localStorageへの履歴保存・削除（最大30件）

src/components/KiriChatPanel.jsx
  └─ 結果画面から開くKiriチャットUI（端末内履歴つき）

docs/DECISION_INPUT_MAP.md
  └─ 判断材料と再現性の対応表

src/lib/chatHistory.js
  └─ チャット履歴の端末内保存・削除（最大40メッセージ）
```

## 3. 実装済み

- 生年月日、出生時刻（任意）、性別（任意）の入力
- 四柱（年柱・月柱・日柱・時柱）の表示
- 日運・時運・月運・年運・大運の表示
- バイオリズム、気分、五行相性を組み合わせたテーマ別スコア
- 色・数字・方角・距離感のヒント
- Kiriのやわらかな読み解き文
- モバイル向けの画面遷移とホワイトアウト演出
- プレミアム案内のUI
- 無料の占い結果と有料チャットの境界表示（購入処理は未接続）
- 結果画面からKiriへ質問する開発プレビュー（占い結果・今日の記録を文脈として送信）
- Kiriチャットの端末内履歴保存・復元・削除
- チャットの短すぎる返答を抑え、続きを求める問いには基本2文でKiri側の要素を添える会話指示へ調整
- Kiriのデザイン基準を、低彩度の夜色・薄い境界線・余白中心の静かなUIへ更新
- 「今日のヒント」をキーワード（色・数字・方角・距離感）と、記録/上位テーマに結びついたKiriの具体的な説明の二段構えへ改善
- 同日・同一人物・同一記録の再分析を安定させるため、ヒント選択を入力シード化し、分析結果をサーバー内で最大200件キャッシュ。Anthropicには`temperature: 0`を指定
- `src/lib/saju.js`に計算を分離し、既知の八字ケースと大運の順逆・起運日を自動検証
- チャット版Kiriの人格定義を占い結果モードのシステム指示に採用

## 4. 未実装・リリースブロッカー

### 最優先: 正確性と安全性

- 四柱推命の計算仕様を専門知識と照合する（特に真太陽時、節入り、時柱、流派ごとの大運）
- 実ユーザーの代表ケースを追加し、外部の命式表との照合を続ける
- 入力値の形式・範囲・本文長をサーバー側でも検証する（分析APIで実装済み）
- APIキーを含むエラー詳細をクライアントへ返さない
- APIの認証と永続的な利用量監視を追加する（IP別レート制限・日次上限・チャットのプレビューゲートはD-08で第一段階済み）
- 医療・金融などの断定を避ける安全文言と危機時の対応を定義する

### 次: プロダクト機能

- アカウント同期を伴う履歴保存（端末内の履歴保存・一覧・削除は実装済み）
- 購読状態に応じたKiriチャットのアクセス制御
- 命式と過去結果をチャットへ安全に渡すコンテキスト設計
- プレミアム案内を実際の購入導線へ接続

### App Store公開前

- Swift/Capacitor等によるiOSアプリの器
- StoreKitまたはRevenueCatによる購入・復元・購読状態確認
- サーバー側のentitlement判定（クライアントのフラグを信用しない）
- プライバシーポリシー、利用規約、サポートページ
- App Store用アイコン、スクリーンショット、説明文
- 実機iPhoneでの音声・キーボード・セーフエリア・通信失敗QA

## 5. 既知の整理事項

- ルート直下の重複`SpiritualDiary.jsx`は削除し、`src/components/SpiritualDiary.jsx`を唯一のUI実装とする
- `next.config.js`は`next.config.mjs`と重複していたため削除
- Create Next Appの未使用SVGと古いfaviconを削除
- `src/app/layout.js`のタイトルと説明をKiri用に更新

## 6. 推奨する実装順

1. 命式計算の基準とテストケースを固定する（済: D-10で境界ケース独立照合・仕様承認）
2. API入力検証・安全性・レート制限を固める（済: D-08で第一段階）
3. 結果を端末またはアカウント単位で保存できるようにする（端末内保存＋JSONバックアップ済み: D-09。アカウント同期は将来）
4. 無料結果と有料チャットの境界を確定する（済: D-11で閲覧境界も確定）
5. 課金・entitlement・チャットを実装する（entitlementスタブ済み。課金はiOS化時: D-12）
6. iOS化、実機QA、プライバシー資料、App Store提出を行う（法務3ページ作成済み・運営者表記は保留。iOSはCapacitor方式確定）

## 7. 検証状態（2026-07-28）

- `npm run build`: 成功
- `npm run lint`: エラーなし（`<img>`の最適化警告が1件）
- `npm test`: 53件成功（命式・境界照合・入力検証・大運・AI応答契約・履歴・API保護・バックアップ）
- `npm audit --omit=dev`: 実行時依存の脆弱性0件

## 8. 今回追加した計算・文章の基準

- 命式は`lunar-javascript`の`getEightChar()`を直接使い、文字列の正規表現抽出を計算の基準にしない
- 大運は`getYun(gender, 2)`の節入りベースの起運日・順逆・大運表を使う
- 性別が未入力/回答拒否/その他の場合、大運の順逆を推測せず判定保留にする
- Kiriの自由チャット人格を土台にし、占い結果では「抽象的な気配」だけで終わらず、今日起こりやすい場面・反応・選択を具体化する
- 断定や脅しは避けるが、`〜かもしれない`だけで逃げず、日主・日運・入力された記録に根ざした読みを最低1つ置く
- AI応答は`deepMessage` / `innerMessage` / `actionAdvice`の3フィールドを必須化し、JSON不正時は失敗扱いにする
- AIエラー詳細や生のモデル出力はクライアントへ返さず、文章上の敬語定型・励まし定型・絵文字過多はサーバー警告として記録する

## 9. 作業記録（2026-07-25）

- `spiritual-diary`を正規本体としてPlaygroundへ移動し、旧チャット試作と混同しない構成に整理
- 重複コンポーネント、重複Next設定、未使用の初期テンプレート資産を削除
- 命式計算を`src/lib/saju.js`へ分離し、`getEightChar()`と節入りベースの大運APIへ移行
- 既知の八字ケース、出生時刻なし、無効入力、大運の男女順逆・起運日をテスト化
- チャット版Kiriの人格を`src/lib/kiriPersonality.js`へ移植し、占い結果モードの具体性ルールを追加
- Next.js / PostCSS / sharpを修正版へ更新し、実行時依存の監査結果を0件にした
- 分析APIの入力検証、AI応答契約、上流エラーの情報漏えい防止を追加
- 入力画面のプレースホルダーは外部API依存をやめ、固定例文で安定表示
- 占い結果を端末の`localStorage`へ最大30件保存し、開始画面から直近5件の確認と個別/全削除を可能にした
- 検証: `npm test` 18件成功、`npm run build`成功、`npm run lint`エラーなし、`npm audit --omit=dev`脆弱性0件
- GitHubブランチ: `agent/consolidate-spiritual-diary`（最新SHAは`git log -1`で確認）

### 未完了の検証

- 外部の命式表・専門家基準との照合ケースはまだ1件のみ
- Anthropic APIを使った実生成文のユーザーテストは未実施
- チャットの購読状態判定・課金接続は未実施（会話履歴は端末内保存を実装済み）
- 端末内履歴の実機確認（Safariプライベートブラウズ、容量超過時を含む）は未実施
- デザインの実機確認（iPhoneの明るさ・文字サイズ変更・ダーク環境）は未実施
- Anthropicの利用可能モデル一覧を確認し、分析APIのモデルを`CLAUDE_MODEL`で切り替え可能にした（未設定時は`claude-sonnet-4-6`）
- 真太陽時、流派ごとの大運差、出生地の扱いは未確定
